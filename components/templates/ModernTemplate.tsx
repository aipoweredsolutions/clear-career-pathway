...
export function ModernTemplate({ data, className, accentColor = 'bg-slate-900' }: TemplateProps) {
    ...
    const sidebarSections = ['skills', 'education', 'languages', 'affiliations']
    const mainSections = data.sectionOrder && data.sectionOrder.length > 0
        ? data.sectionOrder.filter(id => !sidebarSections.includes(id) && id !== 'personalInfo' && id !== 'summary' && id !== 'professionalSummary')
        : [
            'experience',
            'projects',
            'certifications',
    ...
    const sectionMap: Record<string, React.ReactNode> = {
        experience: workExperience && workExperience.length > 0 && (
            <section key="experience">
                <h3 className="text-xl font-black text-slate-900 border-l-[6px] border-slate-900 pl-3 mb-2 uppercase tracking-[0.2em]">Experience</h3>
                <div className="space-y-6">
                    {workExperience.map((job, i) => (
                        <div key={i} className="relative group">
                            ...
                            <div className="text-lg font-semibold text-slate-600 mb-2">{job.companyName}</div>
                            <p className="text-slate-600 mb-2 leading-relaxed">{job.roleDescription}</p>
                            {job.achievements && (
        ...
        projects: projects && projects.length > 0 && (
            <section key="projects" className="mb-6">
                <h3 className="text-xl font-bold text-slate-900 border-l-4 border-slate-900 pl-3 mb-2 uppercase tracking-wider">Projects</h3>
                <div className="space-y-6">
        ...
        certifications: certifications && certifications.length > 0 && (
            <section key="certifications" className="mb-6">
                <h3 className="text-xl font-bold text-slate-900 border-l-4 border-slate-900 pl-3 mb-2 uppercase tracking-wider">Certifications</h3>
        ...
        achievements: achievements && achievements.length > 0 && (
            <section key="achievements" className="mb-6">
                <h3 className="text-xl font-bold text-slate-900 border-l-4 border-slate-900 pl-3 mb-2 uppercase tracking-wider">Achievements</h3>
        ...
        volunteer: volunteerExperience && volunteerExperience.length > 0 && (
            <section key="volunteer" className="mb-6">
                <h3 className="text-xl font-bold text-slate-900 border-l-4 border-slate-900 pl-3 mb-3 uppercase tracking-wider">Volunteering</h3>
        ...
        publications: publications && publications.length > 0 && (
            <section key="publications" className="mb-6">
                <h3 className="text-xl font-bold text-slate-900 border-l-4 border-slate-900 pl-3 mb-4 uppercase tracking-wider">Publications</h3>
        ...
        additional: additionalInfo && (
            <section key="additional" className="mb-6">
                <h3 className="text-xl font-bold text-slate-900 border-l-4 border-slate-900 pl-3 mb-3 uppercase tracking-wider">Additional Info</h3>
        ...
        references: references && references.length > 0 && (
            <section key="references" className="mb-6">
                <h3 className="text-xl font-bold text-slate-900 border-l-4 border-slate-900 pl-3 mb-4 uppercase tracking-wider">References</h3>
        ...
        custom: customSections && customSections.length > 0 && (
            <div key="custom">
                {customSections.map((section, idx) => (
                    <section key={section.id || idx} className="mb-6">
                        <h3 className="text-xl font-bold text-slate-900 border-l-4 border-slate-900 pl-3 mb-3 uppercase tracking-wider">{section.title}</h3>
    ...
    return (
        <div className={cn("w-full bg-white aspect-[210/297] grid grid-cols-12 shadow-sm font-lato", className)}>
            {/* Sidebar - Dark with subtle gradient */}
            <div className={cn("col-span-4 text-white p-6 flex flex-col gap-6 relative overflow-hidden", accentColor)}>
                ...
                <div className="relative z-10 text-center">
                    ...
                    <div className="w-24 h-24 mx-auto bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center mb-6 border-2 border-white/30 shadow-lg">
                        <span className="text-3xl font-bold tracking-tighter">{personalInfo?.fullName?.split(' ').map((n: string) => n[0]).join('')}</span>
                    </div>
            ...
            <div className="col-span-8 p-6 flex flex-col gap-1.5">
                <header className="mb-1">
                    <h1 className="text-6xl font-black text-slate-900 tracking-tighter leading-none">{personalInfo?.fullName}</h1>
                    {professionalSummary && (
                        <p className="mt-1 text-slate-600 leading-relaxed text-lg font-medium max-w-2xl">{professionalSummary.summaryText}</p>
                    )}
                </header>

                {mainSections.map(sectionId => sectionMap[sectionId])}
            </div>
        </div>
    )
}
